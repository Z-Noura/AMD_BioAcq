 ...
 *   on the first sampling period, we acquire the 8 channels
 *   on the 19 following periods, we acquire only channels 1 and 2
 *   Each sample being coded on 2 bytes, these sequences are coded on 2*(19*2+8)=92 bytes
 * Hence, one packet contains 2+2*92 = 186 bytes
 */
